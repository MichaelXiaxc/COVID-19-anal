...
#data=np.array(CountryCode_covid_data['New_cases'])   #获取序列,CountryCode替换为相应国家码
#print("model for new cases")
#data=np.array(CountryCode_covid_data['New_deaths'])   #获取序列
#print("model for new deaths")
#data=np.array(CountryCode_covid_data['Cumulative_cases'])   #获取序列
#print("model for cumulative cases")
#data=np.array(CountryCode_covid_data['Cumulative_deaths'])   #获取序列
    ...
    with tf.Session() as sess:
        ...
        for i in range(100000):
            ...
            while(end<len(train_x)):
                ...
                if step%10==0:
                    print(i,step,loss_)
                    print("保存模型：",saver.save(sess,'stock_Cd.model'))
                step+=1
